...
function App() {
  ...
  const createButton = ({color, text }) =>  (
      <button className={`bg-${color}-500 text-white p-2 m-2 rounded-lg`} onClick={()=> {}}>{text}</button>
    )
  ...
  return (
    <>
      <div className="w-3/4 border border-red-700 h-auto m-auto">
        <div className="flex flex-col justify-center items-center">
          <strong className="text-3xl">Json Builder</strong>
        </div>

        <div className="flex flex-row " id="form">
            {createButton({color: "green", text: "Array"})}
            {createButton({color: "black", text: "Array"})}
        </div>
